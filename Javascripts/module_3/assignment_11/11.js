...
for (let i=0; i<picArray.length; i++) {
    let current = picArray[i];
    let article = document.createElement('article');
    article.setAttribute('class', 'card');
    section.appendChild(article)

    let h2 = document.createElement('h2');
    h2.innerHTML = current.title;
    ...
    img.setAttribute('src', current.image.medium);
    ...
    p_tag.innerHTML = current.description;
    ...
    article.addEventListener(
        ...
        function() {
            ...
            let img1 = document.querySelector('dialog img');
            img1.setAttribute('src', current.image.large);
            img1.setAttribute('alt', current.title);
            dialog.showModal();
        }
    );
